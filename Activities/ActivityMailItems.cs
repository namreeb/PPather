...
namespace Pather.Activities
{
	public class ActivityMailItems : Activity
	{
		GNode mailbox;
		string to;
		bool MailWhites, MailGreens, MailBlues, MailEpics;
		List<string> items;
		bool FullStacksOnly;
		...
		public ActivityMailItems(Task t, GNode mailbox,
								...
								bool FullStacksOnly,
								bool MailWhites, bool MailGreens, bool MailBlues, bool MailEpics)
			: base(t, "Mail")
		{
		...
		private bool ShouldMail(GItem item)
		{
			...
			{
				if (item.Name.Contains(litem))
					return true;
			}
            if (item.Definition.Quality == GItemQuality.Common && MailWhites == true)
                return true;
