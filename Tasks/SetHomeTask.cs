...
namespace Pather.Tasks {
	public class SetHomeTask : NPCInteractTask {
		public const string ParserKeyword = "SetHome,SetHearth";
		private bool done = false;

		public SetHomeTask(PPather pather, NodeTask node)
			: base(pather, node) {
		}
		...
		ActivitySetHome activity;
		public override Activity GetActivity() {
			if (!IsCloseToNPC())
				return GetWalkToActivity();
			else {
				if (activity == null)
					activity = new ActivitySetHome(this, FindNPC());
				return activity;
			}
		}
