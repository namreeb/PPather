...
namespace Pather.Helpers.UI {
	public class TalentFrame {
		...
		public static void SpendPoint(int tab, int spot) {
			Functions.Click(GContext.Main.Interface.GetByName(fTAB+tab));
			Thread.Sleep(250);
			...
			Functions.Click(GContext.Main.Interface.GetByName(fRANK + spot));
		...
		public static string[] GetTalentString() {
			...
			for (int i = 0; i < 3; i++) {
				Functions.Click(GContext.Main.Interface.GetByName(fTAB+(i+1)));
